...
// ============= CAR HISTORY (История автомобилей) =============
export function getCarHistory() {
  try {
    const data = localStorage.getItem('carHistory');
    const history = data ? JSON.parse(data) : [];
    return Array.isArray(history) ? history : [];
  } catch (err) {
    console.warn('Error loading car history', err);
    return [];
  }
}

export function addCarToHistory(carName) {
  if (!carName || carName.trim() === '') {
    return;
  }

  const history = getCarHistory();
  const trimmedCar = carName.trim();
  
  // Проверяем, нет ли уже такого автомобиля (без учета регистра)
  const exists = history.some(car => car.toLowerCase() === trimmedCar.toLowerCase());
  
  if (!exists) {
    const newHistory = [trimmedCar, ...history.slice(0, 19)]; // Оставляем только 20 последних
    localStorage.setItem('carHistory', JSON.stringify(newHistory));
  }
}

export function removeCarFromHistory(carName) {
  if (!carName || carName.trim() === '') {
    return;
  }
  ...
  // Remove car by case-insensitive comparison
  const filtered = history.filter(car => 
    car.toLowerCase() !== trimmedCarName.toLowerCase()
  );
  
  localStorage.setItem('carHistory', JSON.stringify(filtered));
}

export function clearCarHistory() {
